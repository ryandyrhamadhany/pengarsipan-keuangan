    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap justify-between items-center gap-4">
                ...
                <div class="flex gap-2">
                    <form method="POST" action="{{ route('notifications.readAll') }}">
                        @csrf
                        <button
                            class="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium hover:bg-blue-600 transition-all duration-200 shadow-sm hover:shadow flex items-center gap-2">
                    ...
                    <form method="POST" action="{{ route('notifications.deleteRead') }}">
                        ...
                        @method('DELETE')
                        <button
                            class="px-4 py-2 bg-red-500 text-white rounded-lg text-sm font-medium hover:bg-red-600 transition-all duration-200 shadow-sm hover:shadow flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="space-y-3">
            @forelse ($notifications as $notif)
                ...
                <div
                    class="bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border-l-4 {{ $config['border'] }} {{ $notif->is_read ? 'opacity-60' : '' }}">
                    <div class="p-5">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2 mb-2">
                                    <h3 class="font-semibold text-gray-900 text-lg">
                                        ...
                                        @if (!$notif->is_read)
                                            <span
                                                class="inline-block w-2 h-2 bg-blue-600 rounded-full ml-2 animate-pulse"></span>
                                        @endif
                                    </h3>
                                    <span
                                        class="{{ $config['badge'] }} px-3 py-1 rounded-full text-xs font-semibold uppercase">
                                        {{ ucfirst($notif->type) }}
                                    </span>
                                </div>
                                    ...
                                    <div class="flex gap-2">
                                        @if (!$notif->is_read)
                                            <form method="POST"
                                                ...
                                                @csrf
                                                <button
                                                    class="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-100 transition-all duration-200 flex items-center gap-1">
                                        ...
                                        @if ($notif->is_read)
                                            <form method="POST"
                                                ...
                                                @method('DELETE')
                                                <button
                                                    class="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-xs font-medium hover:bg-red-100 transition-all duration-200 flex items-center gap-1">
            ...
            @empty
                <div class="bg-white rounded-xl p-28 text-center shadow-md">
